...
当前已完成 B01 至 B10、C1-A 至 C1-C，以及 C1-R 的 R0 至 R6。最新工作统一了逐帧控制入口、战斗与导航会话、外力恢复和带速度的一格跨隙，补齐了已验证动作在缺锚点、路线依赖变化和新路线交接时的身体责任，并让失败分类真正改变 Runtime 生命周期、让 Runtime 持续维护唯一世界知识。正式 Fabric 结果与适用边界写在 `docs/motion_navigation/acceptance/`。
...
这六个批次分别覆盖 B10-C 跨隙、C1-B 移动近战和 C1-C 外力恢复，每类各有一个完整通过批次和一个完整失败批次。验证器会检查归档哈希、安全边界、完整试次数量、汇总和失败分类。压缩归档合计不超过 30 MiB。
...
- 公开仓库附带六个结构化真实运行批次，可以重新统计对应的通过和失败结果。

## 不能说明什么

这个快照不包含世界存档、完整普通日志、画面、Gradle 缓存、Minecraft 依赖 JAR 或构建产物，因此不能只靠本仓库重跑游戏内正式实验。公开的六个真实批次是经过筛选的结构化轨迹，可以核对历史结果和读取链，但不能替代新的 Fabric 实机运行。其他原始证据仍由主项目保管。
