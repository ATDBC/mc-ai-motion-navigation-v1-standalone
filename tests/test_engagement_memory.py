...
class EngagementMemoryTests(unittest.TestCase):
    ...
    def test_regression_gap_and_expiry_never_drive_navigation(self):
        ...
        gap_obs = observation(7, visible=False, client_completed_ns=7000)
        waiting = advance_engagement(
            first, event_from_observation(identity, gap_obs),
        )
        self.assertTrue(waiting.active)
        self.assertTrue(waiting.awaiting_continuity_reanchor)
        self.assertEqual(waiting.revocation_reason, "observation_gap")
        self.assertIsNone(resolve_target_position(
            waiting, gap_obs, identity, gap_obs.received_at_monotonic_ns,
        ))
        resumed_obs = observation(8, visible=False, client_completed_ns=8000)
        resumed = advance_engagement(
            waiting, event_from_observation(identity, resumed_obs),
        )
        self.assertTrue(resumed.active)
        self.assertFalse(resumed.awaiting_continuity_reanchor)
        self.assertIsNone(resumed.revocation_reason)
