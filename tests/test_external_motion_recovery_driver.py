...
class ExternalMotionRecoveryDriverTests(unittest.TestCase):
    ...
    def test_one_neutral_owner_covers_air_and_ground_without_overlap(self):
        ...
        self.assertEqual(records.count("ordered_source_registered"), 1)
        self.assertEqual(records.count("ordered_source_unregistered"), 0)
        self.assertEqual(self.driver.report.state, "braking")
        self.assertEqual(self.runtime.ordered_source_stats["active_sources"], 1)

    def test_two_stable_ticks_release_owner_on_the_next_driver_tick(self):
        self.driver.start(damage_event())
        self.tick()
    ...
    def test_same_owner_survives_a_second_airborne_interval(self):
        self.backend.frames = (
            (31, False, .10),
            (32, True, .08),
            (33, False, .10),
        ...
        self.clock[0] += 50_000_000
        continued = self.tick()
        self.assertEqual(continued.decision.action.movement, MovementV1())
        self.assertEqual(self.runtime.ordered_source_stats["active_sources"], 1)
