...
(async()=>{
  ...
  try{
    ...
    assert.match(await page.locator('#status').innerText(),/�Ѱ� minecraft-java-1_21-player-motion-r1 ����/);
    ...
    for(const id of ['top-view','side-view','error-view']){
      ...
      assert.ok(size.width>100&&size.height>100,`${id} δ����`);
    }
    ...
    assert.notEqual(await page.locator('#frame').innerText(),before,'����ʱ tick Ӧǰ��');
    await page.locator('#sequence').selectOption({index:1});
    assert.match(await page.locator('#segment-reason').innerText(),/�ж�ԭ��/);
    const modes=await page.locator('#run option').allTextContents();
    const airIndex=modes.findIndex(value=>value.includes('B09 ����ת��'));
