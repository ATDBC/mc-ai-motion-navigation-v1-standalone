...
B11 复审又补出一个相邻边界：伤害出现后，如果下一段残差因为缺世界格而无法计算，更晚的偏差不能被改记成无来源外力。现在检测器保留伤害事实；补齐紧邻证据时正常归因，跨多 tick 的匹配残差可以证明没有预测外运动，跨多 tick 的偏差或等待到期则产生 `DAMAGE_WITH_UNVERIFIED_MOTION` 并进入保守恢复。原复现最终得到 `damage_motion_unverified_span`；当前公开 C1-C 批次中的相邻真实时序得到 `damage_knockback_confirmed`。
