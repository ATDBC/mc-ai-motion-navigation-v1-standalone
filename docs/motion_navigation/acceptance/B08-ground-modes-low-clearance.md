...
表面规划现可显式使用 Walk、Sprint、Crouch 或 Crawl Profile。自动检查证明：低通道中站立图不生成通路，Crawl 图使用 0.6 格身体高度生成通路；Sprint 图使用实测速率并在 7 点饥饿值处准入，6 点时拒绝。相同模式配置通过后台快照进程后仍保留。全量运动／导航 211 项、观察协议 19 项和 Fabric 探针构建均通过。

一次请求仍只选择一个明确地面模式。混合模式自动选路属于 B14，不在本补充结论中。
