...
最终复审确认上一轮问题和实机证据均已闭合，又指出三个实现边界和两处记录偏差。本轮完成以下修正：

1. 已验证动作的最早、最晚生效 tick 绑定在移动意图上。仲裁器只把获胜意图的窗口交给 Runtime，战斗和 B11 使用的正式导航驱动不再退化成单 tick 窗口。
2. 带证明窗口的移动意图只使用一次，避免下一帧重放已经过期的命令和窗口。
3. 身体同时覆盖多个支撑面时，先选择脚面高度最接近的支撑面，再比较水平距离。整格方块边缘不会被旁边较低半砖抢走起点。
4. 伤害归因只允许伤害当帧或紧邻下一帧的单段偏差形成已确认击退。横跨多个 tick 的偏差只能形成“伤害发生、运动未验证”的保守事件。
5. D024、C1-R6 和当前阶段记录改用现行公开批次，旧批次只保留在历史公开提交中。

新的正式 Fabric 回归如下：

| 场景 | 结果 | 关键数据 |
|---|---:|---|
| B10 `20260925T134940922188Z-a059bdc6` | 40／40 基础跨隙、40／40 转弯出口、60／60 三档带速、10／10 正式协调入口 | 200 条已验证命令均在许可窗口内；142 次求解 P99 25.8174 ms |
| C1-C `20260925T135809856138Z-ab9ef883` | 20／20 正例、10／10 反例 | 25 次真实外力恢复；控制 P95 6.6656 ms、P99 8.0734 ms；恢复响应 P99 49.3024 ms |

B10 本轮的首条命令仍全部在最早 tick 生效，因此“允许晚一个 tick”的正式导航路径由确定性 Runtime 集成测试覆盖，不能把本次实机通过误写成晚 tick 的实机证据。
