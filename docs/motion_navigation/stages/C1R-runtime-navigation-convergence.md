...
- 目标已有受伤动画时仍可在其他条件满足后攻击。命中确认改为比较攻击前后的动画计时变化；
...
4. 把“是否允许再次攻击”和“上一击是否已经确认”分开。目标受伤动画可以帮助确认命中，但队友攻击、着火或其他伤害造成的动画不能持续阻止新的合法攻击。
...
- Fabric 客户端会在玩家实际输入采样前重新检查目标身份、准星、距离、冷却、界面和世界状态。攻击与移动在同一个玩家运动 tick 生效后，客户端才返回本次请求的回执；
...
- 伤害事实已同时检查生命和吸收心。只有伤害事实与覆盖伤害 tick 的完整运动残差同时成立，才形成 `DAMAGE_KNOCKBACK`；没有伤害来源的预测外运动单独报告为 `UNATTRIBUTED_EXTERNAL_MOTION`；
