...
4. [参照校验](../../../mc2p/motion_nav/evidence/reference_baselines.py)逐文件检查快照，拒绝缺失、修改和未登记文件。
5. [证据归一化](../../../mc2p/motion_nav/evidence/reference_evidence.py)只读现有封存记录，导出 `inputs.jsonl`、`body.jsonl`、`timings.jsonl` 和 `run.json`；摘要同时保留原始场景编号，避免把旧成绩错贴到共同场景。
