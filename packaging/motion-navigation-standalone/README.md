...
当前已完成 B01 至 B10、C1-A 至 C1-C、C1-R 的 R0 至 R6，以及 B11 的固定放置与有限搭桥。最新整改让动作证明的生效窗口随获胜移动意图进入 Runtime，修正了边缘支撑面的高度选择，并把无法定位到相邻 tick 的伤害偏差转入保守恢复。正式 Fabric 结果与适用边界写在 `docs/motion_navigation/acceptance/`。
